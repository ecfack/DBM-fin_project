...
<?php
  ...
  $temp = "0";
    ...
    if ($total_records > 0 && $row[7] > 0){
      $_SESSION["login_session"] = false;
      header("Location:bucketpage.html");
      exit;
    }
    ...
    if ( $total_records > 0 ) {
        ...
        $_SESSION["identity"] = $row[6];
        $_SESSION["bucket"] = $row[7];
        $_SESSION["image"] = $row[8];
        $_SESSION["intro"] = $row[9];
        $_SESSION["level"] = $row[10];

        echo "<script> {window.alert('Link start!');
...
<form method="post" action="login.php">
        ...
        <div class="group">
          <label for="password" class="label">Password</label>
          <input id="password" type="password" name="password" class="input" data-type="password">
          <label class="label"><font color="red"><b>Incorrect id or password</b></font></label>
        </div>
        <div class="group">
          <input id="check" type="checkbox" class="check" checked>
          <label for="check"><span class="icon"></span> Keep me Signed in</label>
